...
💬 教練說
- 嚴格依照「今日最值得提的角度」的 →指引寫 1 句具體話。
- 不要重複總結中已出現的數字。
- 不要用「進度不錯」「繼續加油」這類空泛鼓勵。
- 如果上方角度為空，就用 1 句具體觀察今天最特別的地方（例：某餐熱量結構、訓練 vs 飲食搭配），不要再講長期體脂趨勢。
"""

WEEKLY_INSIGHT_PROMPT = """\
根據本週數據與三個面向的分析角度，寫教練評論。每個面向最多 1 行，最多 50 字。
...
格式規則：
- 不要 markdown（不要 **粗體**、## 標題）
- 純文字 + emoji
- 每行嚴格依照對應角度的「→」指引寫
- 不要重複本週總結已有的數字（不要再說「平均 1381kcal」這種）
- 不要罐頭話（不要「進度不錯」、「繼續加油」、「持續努力」、「持續多攝取 XX 等優質 OO」）
- 不要恭維已 100% 達標的面向 — 那不算「值得提的角度」
- 如果某個角度為空字串，**該行整行省略**（不要寫「無觀察」之類）
- 一行一個面向，順序：運動、飲食、身體
